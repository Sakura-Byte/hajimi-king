...
class OptimizedAsyncProcessor:
    ...
    async def add_file_task(self, item: Dict[str, Any], query: str, priority: Optional[int] = None) -> bool:
        """添加文件处理任务"""
        if not self.is_running:
            return False
    ...
    async def _download_file_content_optimized(self, item: Dict[str, Any], token: str, worker_id: str) -> Optional[str]:
        """优化的文件内容下载"""
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                async with session.get(metadata_url, headers=headers, proxy=proxy_config.get('http') if proxy_config else None) as response:
                    ...
                    async with session.get(download_url, headers=headers, proxy=proxy_config.get('http') if proxy_config else None) as content_response:
    ...
    async def _validate_gemini_key_async(self, api_key: str, worker_id: str) -> str:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, 
                self._validate_gemini_key_sync, 
                api_key
            )
            return result
