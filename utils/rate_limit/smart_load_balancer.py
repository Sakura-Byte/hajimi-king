...
class PredictiveLoadBalancer:
    ...
    def _initialize_resource_health(self):
        ...
        for token in self.quota_monitor.github_tokens:
            self.resource_health[token] = ResourceHealth(resource_id=token, resource_type=ResourceType.GITHUB_TOKEN)

        # Proxies
        for i, proxy in enumerate(Config.PROXY_LIST):
            proxy_id = f"proxy_{i}_{hash(proxy) % 1000}"
            self.resource_health[proxy_id] = ResourceHealth(resource_id=proxy_id, resource_type=ResourceType.PROXY)

        # Gemini (简化为单一资源)
        self.resource_health["gemini_primary"] = ResourceHealth(
            resource_id="gemini_primary", resource_type=ResourceType.GEMINI_KEY
        )

    async def get_best_github_token(
        self, task_priority: int = 5, predicted_load: float = 1.0
    ) -> LoadBalancingDecision | None:
        """
        获取最佳GitHub token
        ...
        self.resource_health[best_token].load_factor = min(
            1.0, self.resource_health[best_token].load_factor + predicted_load * 0.1
        )

        return decision

    async def get_best_proxy(self, task_context: str = "") -> LoadBalancingDecision | None:
        """获取最佳代理"""
    ...
    async def _handle_github_token_failover(self, task_priority: int) -> LoadBalancingDecision | None:
        """处理GitHub token故障转移"""
