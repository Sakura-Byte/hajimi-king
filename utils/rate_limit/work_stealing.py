...
class WorkerInfo:
    ...
    current_task: Optional[ScheduledTask] = None
...
class WorkStealingScheduler:
    ...
    def register_worker(self, worker_id: str, worker_type: TaskType, preferred_token: Optional[str] = None) -> bool:
        """
        注册新worker
        ...
        with self.workers_lock:
            ...
            logger.info(f"👷 Registered {worker_type.value} worker {worker_id} with token {assigned_token[:10] if assigned_token else 'None'}...")
            return True
    ...
    def assign_task_to_worker(self, task: ScheduledTask) -> Optional[str]:
        """
        将任务分配给最合适的worker
    ...
    def get_next_task_for_worker(self, worker_id: str) -> Optional[ScheduledTask]:
        """
    ...
    def _assign_token_to_worker(self, worker_id: str, worker_type: TaskType, preferred_token: Optional[str]) -> Optional[str]:
        """为worker分配token"""
    ...
    def _is_token_compatible_for_task(self, worker_token: Optional[str], task: ScheduledTask) -> bool:
    ...
    def _attempt_work_stealing(self, stealing_worker_id: str) -> Optional[ScheduledTask]:
        """
        尝试工作窃取
    ...
    def _steal_task_from_worker(self, target_worker: WorkerInfo, stealing_worker: WorkerInfo) -> Optional[ScheduledTask]:
        """从目标worker窃取任务"""
    ...
    def _redistribute_task(self, task: ScheduledTask, exclude_worker: Optional[str] = None):
        """重新分配任务"""
        # 选择对应类型的worker
        if task.task_type == TaskType.GITHUB_FILE:
            candidate_workers = self.github_workers
    ...
    def get_scheduler_stats(self) -> Dict:
        ...
        with self.workers_lock:
            ...
            if total_workers > 0:
                ...
                queue_variance = sum((worker.queue_size - avg_queue_size) ** 2 for worker in self.workers.values()) / total_workers
