...
class RateLimitBackpressure:
    ...
    def _calculate_github_pressure(self) -> float:
        ...
        if github_summary['total_tokens'] > 0:
            ...
            if avg_remaining < self.github_critical_threshold:
                ...
                pressure_factors.append(max(0, (self.github_warning_threshold - avg_remaining) / self.github_warning_threshold))
    ...
    def should_pause_github_tasks(self) -> bool:
        """是否应该暂停GitHub任务"""
        return (
            self.current_state.level == BackpressureLevel.CRITICAL or
            self.current_state.github_pressure > 0.85
        )
    ...
    def should_pause_gemini_tasks(self) -> bool:
        ...
        return (
            self.current_state.level == BackpressureLevel.CRITICAL or
            self.current_state.gemini_pressure > 0.8
        )
    ...
    def get_max_concurrent_github_workers(self) -> int:
        ...
        if self.current_state.level == BackpressureLevel.NONE:
            return base_workers
        elif self.current_state.level == BackpressureLevel.LOW:
            return max(2, int(base_workers * 0.8))
        elif self.current_state.level == BackpressureLevel.MEDIUM:
            return max(2, int(base_workers * 0.6))
        elif self.current_state.level == BackpressureLevel.HIGH:
