...
class FileTaskInfo:
    ...
    def estimated_key_density(self) -> float:
        ...
        if not self.content_preview:
            ...
            if self.file_path.endswith(('.js', '.ts', '.py', '.env', '.config')):
                return 0.7
            elif self.file_path.endswith(('.json', '.yaml', '.yml')):
                return 0.8
            elif self.file_path.endswith(('.md', '.txt', '.rst')):
                return 0.2
            else:
                return 0.5
    ...
    def repo_popularity_score(self) -> float:
        ...
        if self.repo_stars >= 10000:
            return 1.0
        elif self.repo_stars >= 1000:
            return 0.8
        elif self.repo_stars >= 100:
            return 0.6
        elif self.repo_stars >= 10:
            return 0.4
        else:
            return 0.2
...
class KeyValidationTaskInfo:
    ...
    def key_confidence_score(self) -> float:
        ...
        # 负面指标
        if any(keyword in context_lower for keyword in ['example', 'sample', 'demo', 'test', 'placeholder', 'your_api_key']):
            return 0.1
...
class PriorityCalculator:
    ...
    def calculate_file_task_priority(
        ...
        current_quota_status: Dict,
        task_age_seconds: float = 0
    ) -> int:
        """
        计算文件任务优先级
...
class QuotaAwarePriorityQueue:
    ...
    def __init__(self, queue_type: str, max_size: int = 1000):
        ...
        self.stats = {
            'total_added': 0,
            'total_removed': 0,
            'priority_distribution': {i: 0 for i in range(10)},
    ...
    def add_file_task(
        ...
        # 计算优先级
        priority = self.priority_calculator.calculate_file_task_priority(
            task_info, current_quota_status
        )
    ...
    def add_validation_task(
        ...
        # 计算优先级
        priority = self.priority_calculator.calculate_validation_task_priority(
            task_info, current_quota_status
        )
    ...
    def get_next_task(self) -> Optional[Tuple[str, Any]]:
        """
        获取下一个任务
    ...
    def get_queue_stats(self) -> Dict:
        """获取队列统计信息"""
        priority_counts = {f"priority_{i}": count for i, count in self.stats['priority_distribution'].items() if count > 0}
