...
class TokenManager:
    ...
    def __init__(self, quota_monitor: QuotaMonitor):
        ...
        self.token_weights: Dict[str, float] = {token: 1.0 for token in self.github_tokens}
        
        # 代理管理
        self.proxy_list = Config.PROXY_LIST.copy() if Config.PROXY_LIST else []
        self.proxy_performance: Dict[str, float] = {proxy: 1.0 for proxy in self.proxy_list}
        
        # 休眠中的token
        self.sleeping_tokens: Dict[str, float] = {}  # token -> wake_up_time
        
        logger.info(f"🎯 TokenManager initialized with {len(self.github_tokens)} GitHub tokens and {len(self.proxy_list)} proxies")
    
    def get_best_github_token(self, task_priority: int = 5) -> Optional[str]:
        """
        获取最佳GitHub token
        ...
        if not available_tokens:
            ...
            if self.sleeping_tokens:
                ...
                logger.warning(f"⏰ All tokens unavailable, will use {best_token[:10]}... (wakes up in {self.sleeping_tokens[best_token] - current_time:.0f}s)")
                return best_token
            else:
                # 从配额状态选择最好的
    ...
    def _calculate_token_score(self, token: str, quota_status, task_priority: int) -> float:
        ...
        final_score = base_score + health_bonus - error_penalty - consecutive_error_penalty - usage_penalty + priority_bonus
