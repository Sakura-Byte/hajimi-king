...
class AdaptiveScheduler:
    ...
    async def get_next_github_task(self) -> Optional[ScheduledTask]:
        """获取下一个GitHub任务"""
    ...
    async def get_next_gemini_task(self) -> Optional[ScheduledTask]:
        """获取下一个Gemini任务"""
    ...
    def clear_old_tasks(self, max_age_hours: int = 1):
        ...
        self.github_queue = [
            task for task in old_github_queue 
        ...
        self.gemini_queue = [
            task for task in old_gemini_queue
            if current_time - task.created_time < max_age_seconds
        ]
        heapq.heapify(self.gemini_queue)
        
        removed_count = (len(old_github_queue) - len(self.github_queue)) + (len(old_gemini_queue) - len(self.gemini_queue))
