...
class QuotaStatus:
    ...
    last_429_time: float | None = None  # 最后一次429错误时间
    ...
    def is_healthy(self) -> bool:
        """Token是否健康"""
        return (
            self.remaining_ratio > 0.1  # 剩余>10%
            and self.error_rate < 0.2  # 错误率<20%
            and (not self.last_429_time or time.time() - self.last_429_time > 300)  # 5分钟内无429
        )
...
class QuotaMonitor:
    ...
    def __init__(self):
        ...
        for token in self.github_tokens:
            self.github_quota_status[token] = QuotaStatus(
                remaining=5000, limit=5000, reset_time=int(time.time()) + 3600
            )
    ...
    async def update_github_quota(self, token: str, headers: dict[str, str]):
        ...
        try:
            ...
            if quota_ratio < self.github_critical_threshold:
                logger.warning(
                    f"🔴 GitHub token {token[:10]}... critical quota: {remaining}/{limit} ({quota_ratio * 100:.1f}%)"
                )
            elif quota_ratio < self.github_warning_threshold:
                logger.warning(
                    f"🟡 GitHub token {token[:10]}... low quota: {remaining}/{limit} ({quota_ratio * 100:.1f}%)"
                )
    ...
    def record_gemini_call(self, success: bool, response_time: float, is_429: bool = False):
        ...
        # 更新平均响应时间
        alpha = 0.1
        self.gemini_metrics.avg_response_time = (
            alpha * response_time + (1 - alpha) * self.gemini_metrics.avg_response_time
        )

    def get_best_github_token(self) -> str | None:
        """获取最佳GitHub token"""
        ...
        if not token_scores:
            ...
            return min(self.github_quota_status.keys(), key=lambda t: self.github_quota_status[t].reset_time)
    ...
    def get_quota_summary(self) -> dict:
        ...
        github_summary = {
            ...
            "avg_health_score": sum(status.health_score for status in self.github_quota_status.values())
            / len(self.github_quota_status)
            if self.github_quota_status
            else 0,
            "tokens_in_danger": sum(
                1
                for status in self.github_quota_status.values()
                if status.remaining_ratio < self.github_warning_threshold
            ),
        }
    ...
    def should_reduce_github_workers(self) -> bool:
        ...
        return (
            summary["github"]["avg_health_score"] < 0.3
            or summary["github"]["healthy_tokens"] < len(self.github_tokens) * 0.5
            or summary["github"]["tokens_in_danger"] > len(self.github_tokens) * 0.7
        )

    def should_reduce_gemini_workers(self) -> bool:
        """是否应该减少Gemini worker数量"""
        return (
            self.gemini_metrics.error_rate > 0.3
            or (self.gemini_last_429 and time.time() - self.gemini_last_429 < 60)  # 1分钟内有429
        )
    ...
    def get_recommended_github_workers(self) -> int:
        ...
        # 根据剩余配额调整
        avg_remaining_ratio = (
            summary["github"]["total_remaining"] / (len(self.github_tokens) * 5000) if self.github_tokens else 0
        )
